...
public class PrefixCommand extends SubCommand {
    ...
    public PrefixCommand(Guilds plugin) {
        super(plugin);
    }
    ...
    public void execute(CommandSender commandSender, String[] strings) {
        ...
        if (sender.hasPermission("Guilds.setPrefix")) {
            if (strings.length >= 3) {
                ...
                if (prefix.length() <= plugin.getGuildsConfig().getPrefixLength()) {
                    if (uuid != null) {
                        if (guild != null) {
                            if (targetGuild != null && guild.equals(targetGuild)) {
                                ...
                                if (rank.isLeader() || rank.canPromote()) {
                                    plugin.getGuildManager().setMemberPrefix(guild.getMember(uuid), prefix);
                    ...
                    plugin.getChat().sendMessage(sender, plugin.getGuildsConfig().getText("error.prefixTooLong"));
                }
            } else {
                plugin.getChat().sendMessage(sender, plugin.getGuildsConfig().getText("error.wrongUsage.prefix"));
            }
        } else {
