...
public class BlackboardMessage {
    ...
    /**
     * creates a new blackboard message
     */
    public void save() {
    ...
    /**
     * puts this into a nice Message
     *
     * @return a nice message
     */
    public String toMessage() {
        OfflinePlayer player = Guilds.getInstance().getServer().getOfflinePlayer(sender);
        String name = ((player != null) && (player.getName() != null)) ? player.getName() : GuildsConfig.getText("error.unknown");
        return GuildsConfig.getColoredText("info.guild.blackboard.message", guild.getColor())
                .replace("{0}", new SimpleDateFormat("dd.MM.yy HH:mm").format(timestamp))
                .replace("{1}", name)
                .replace("{2}", message);
    }
}
